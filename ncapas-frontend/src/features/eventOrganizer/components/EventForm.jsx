...
export default function EventForm({
  ...
  const seatingConfig = {
    vipPrice: seats.VIP?.price || 0,
    generalPrice: seats.GENERAL?.price || 0,
    vipSeats: seats.VIP?.quantity || 0,
    generalSeats: seats.GENERAL?.quantity || 0,
  };
...
<div className="border-t border-neon-blue/20 pt-6">
  ...
  <button
    type="button"
    onClick={addDiscount}
    className="mb-4 rounded-lg bg-neon-blue px-4 py-2 text-sm font-semibold text-white hover:bg-neon-blue/80"
  >
    + Añadir descuento
  </button>
  ...
  {discounts.map((d, index) => (
    <div
      key={index}
      className="mb-4 space-y-3 rounded-lg border border-neon-purple/20 bg-muted/20 p-4"
    >

      <div className="grid gap-3 md:grid-cols-2">
        <div>
          <label className="block text-xs text-foreground/60 mb-1">Categoría</label>
          <select
            value={d.category}
            onChange={(e) =>
              handleDiscountChange(index, "category", e.target.value)
            }
            className="w-full rounded-lg border border-neon-blue/20 bg-background/70 p-2 text-sm text-white outline-none focus:border-neon-blue/60"
          >
        ...
        <div>
          <label className="block text-xs text-foreground/60 mb-1">Tipo</label>
          <select
      ...
      <div>
        <label className="block text-xs text-foreground/60 mb-1">Valor</label>
        <input
          type="number"
          value={d.value}
          onChange={(e) =>
            handleDiscountChange(index, "value", Number(e.target.value))
          }
          className="w-full rounded-lg border border-neon-blue/20 bg-background/70 p-2 text-sm text-white outline-none focus:border-neon-blue/60"
        />
      </div>
      ...
      {d.category === "CODE" && (
        <div>
          <label className="block text-xs text-foreground/60 mb-1">Código</label>
          <input
            value={d.code}
            onChange={(e) =>
      ...
      <div>
        ...
        <input
          ...
          value={d.minTickets}
          onChange={(e) =>
            handleDiscountChange(index, "minTickets", Number(e.target.value))
          }
          className="w-full rounded-lg border border-neon-blue/20 bg-background/70 p-2 text-sm text-white outline-none focus:border-neon-blue/60"
        />
      </div>
...
<div className="relative">
  <input
    ...
    value={d.validUntil}
    onChange={(e) =>
      handleDiscountChange(index, "validUntil", e.target.value)
    }
    className="w-full rounded-lg border border-neon-blue/20 bg-background/70 p-2 pr-10 text-sm text-white outline-none focus:border-neon-blue/60 cursor-pointer"
    id={`date-${index}`}
  />
  ...
  <button
    type="button"
    onClick={() => {
      document.getElementById(`date-${index}`)?.showPicker();
    }}
